...
void gdiam_approx_diam_pair_wrapper( gdiam_real  * start, 
                                int  size, gdiam_real *p, 
                                gdiam_real *q)
{
    gdiam_point  * p_arr;
    GPointPair  pair;
    ...
    pair = gdiam_approx_diam( p_arr, size, 0.0);
    free( p_arr );

    memcpy(p, pair.p, 3*sizeof(gdiam_real));
    memcpy(q, pair.q, 3*sizeof(gdiam_real));
